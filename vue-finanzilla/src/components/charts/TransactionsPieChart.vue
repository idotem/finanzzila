...
watch(
    () => props.transactionsProp,
    (newValue: Transaction[], oldValue) => {
...
function groupTransactions(tr: Transaction[]) {
    const totalAmount = tr.reduce(
        (acc, curr) =>
            curr.category.name !== 'Income' ? acc + Math.abs(curr.amount) : acc,
        0
    );

    groupedTransactions.value = tr.reduce(
    ...
    const categoriesForPieReduced = Object.values(
        groupedTransactions.value
    ).map((tr) => tr.percentFromTotal + '% ' + tr.categoryName);
    const totalAmountsByCategory = Object.values(groupedTransactions.value).map(
        (tr) => tr.totalAmount
    );
    data.value = {
...
const options: any = {
    ...
    onClick: function (event: any, elements: any) {
        ...
        router.push({
            name: 'Transactions',
            query: { categoryId: categoryId, dateFilter: props.dateFilter }
        });
    }
};
</script>
